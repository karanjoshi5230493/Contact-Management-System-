...
//UpdDelRemForm class updates or deletes reminder entry.
public class UpdDelRemForm extends JFrame implements ActionListener, ItemListener
{
	...
	int confirm=0;
  	public UpdDelRemForm(String fname)
  	{
    	super(fname);
		...
		if(c.size()>0)
		{
			...
			try
			{
				...
				txt_Remarks.setText(token.nextToken());
				br.close();
		...
		panel2.add(new JLabel("Events Inc."));
		panel3.setLayout(new FlowLayout());
		panel3.add(new JLabel("Update Delete Reminder Form"));
		...
		panel4.add(txt_Name);
		panel4.add(new JLabel("Enter reminder text: "));
		panel4.add(txt_Remarks);
		panel5.setLayout(new FlowLayout(FlowLayout.LEFT));
		panel5.add(new JLabel("Select Date of Reminder : "));
    	...
    	if(ae.getSource()==cmd_Update)
    	{
			try
			{
	    		...
	    		while((str=br.readLine())!=null)
	    		{
		    		...
		    		{
			    		String s=txt_Entryid.getText().trim()+"*"+txt_Name.getText().trim()+"*"+(String)cho_Day.getSelectedItem()+"*"+(String)cho_Month.getSelectedItem()+"*"+txt_Year.getText().trim()+"*"+txt_Remarks.getText().trim();
		    			bw.write(s+"\n");
		    		}
      		...
      		try
			{
				...
				while((str=br.readLine())!=null)
				{
					...
					if(item.equals(name))
					{
						...
						String rem=token.nextToken();
						txt_Entryid.setText(eid);
						txt_Name.setText(name);
						txt_Year.setText(yy);
						txt_Remarks.setText(rem);
