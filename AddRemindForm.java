...
//AddRemindForm class adds a new reminder date entry.
public class AddRemindForm extends JFrame implements ActionListener
{
  ...
  private JTextField txt_Entryid,txt_Name,txt_Year,txt_Remarks;
  private JButton cmd_Add,cmd_Cancel,cmd_New;

  public AddRemindForm(String fname)
  {
    super(fname);
    ...
    panel.add(new JLabel("Events Inc."));
    panel1.add(new JLabel("Reminder Entry Form"));
    tpanel.setLayout(new BorderLayout());
    tpanel.add(panel,BorderLayout.NORTH);
    tpanel.add(panel1,BorderLayout.SOUTH);
    
    
    panel2.setLayout(new GridLayout(3,2,0,5));
    panel2.add(new JLabel("Entry ID : "));
    panel2.add(txt_Entryid);
    panel2.add(new JLabel("Name : "));
    panel2.add(txt_Name);
    panel2.add(new JLabel("Enter reminder text: "));
    panel2.add(txt_Remarks);
    panel3.setLayout(new FlowLayout(FlowLayout.LEFT));
    panel3.add(new JLabel("Enter Date of Rem. : "));
  ...
  public void actionPerformed(ActionEvent ae)
  {
	  ...
	  if(ae.getSource()==cmd_Add)
	  {
		  ...
		  if(checkValues()==true)
		  {
			  ...
			  try
			  {
				  //Open reminder file to append a new entry.
				  BufferedWriter br=new BufferedWriter(new FileWriter("Reminder",true));
				  String str=new String();
				  if(txt_Remarks.getText().trim().length()==0)
				  {
					  txt_Remarks.setText("NIL");
					  
				  }
				  str=txt_Entryid.getText().trim()+"*"+txt_Name.getText().trim().toUpperCase()+"*"+(String)cho_Day.getSelectedItem()+"*"+(String)cho_Month.getSelectedItem()+"*"+txt_Year.getText().trim()+"*"+txt_Remarks.getText().trim().toUpperCase()+"\n";
				  br.write(str);
				  br.close();
